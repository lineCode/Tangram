...
void MessagePumpForUI::DoRunLoop() {
  ...
  TangramCommon::ITangramDelegate* pDelegate = nullptr;
  if (g_pTangramImpl&&g_pTangramImpl->m_dwThreadID == ::GetCurrentThreadId())
  {
	  pDelegate = g_pTangramImpl->m_pTangramDelegate;
  }
  ...
  for (;;) {
	...
	//if(g_pTangramImpl && pDelegate && pDelegate->m_pJVMenv &&g_pTangramImpl->m_bIsEclipseInit==false)
	//	continue;
	// end Add by TangramTeam
	...
	if (pDelegate) {
		if (g_pTangramImpl->m_nAppType != TANGRAM_APP_BROWSER &&
			g_pTangramImpl->m_nAppType != TANGRAM_APP_ECLIPSE &&
			g_pTangramImpl->m_nAppType != TANGRAM_APP_BROWSER_ECLIPSE &&
			g_pTangramImpl->m_hMainWnd == 0)
		{
			state_->should_quit = true;
			break;
		}
		if (g_pTangramImpl->m_nAppType == TANGRAM_APP_BROWSERAPP &&
			::IsWindow(g_pTangramImpl->m_hMainWnd )== false)
		{
			state_->should_quit = true;
			break;
		}
