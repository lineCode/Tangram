...
typedef TangramCommon::CTangramImpl*(
    __stdcall* GetTangramImplFunction)();
...
public:
	...
	virtual int OnDestroyMainFrame(CString strID, int nMainFrameCount, int nWndType) = 0;
	virtual LRESULT OnForegroundIdleProc() = 0;
	virtual BOOL TangramPreTranslateMessage(MSG* pMsg) = 0;
	virtual void OnTangramClose() = 0;
	virtual void OnExtendComplete(HWND hWnd, CString bstrUrl, IWndNode* pRootNode) = 0;
	virtual void OnTangramEvent(ITangramEventObj* NotifyObj) = 0;
	virtual void RegistWndClassToTangram() = 0;
	virtual void OnActiveDocument(ITangramDoc* ActiveDoc, IWndNode* pNodeInDoc, IWndNode* pNodeInCtrlBar, HWND hCtrlBar) = 0;
	virtual HWND CreateNewFrame(CString strFrameKey) = 0;
	virtual HWND GetActivePopupMenu(HWND) = 0;
	virtual HRESULT CreateTangramCtrl(void* pv, REFIID riid, LPVOID* ppv) = 0;
	virtual ITangramDoc* CreateNewDocument(LPCTSTR lpszFrameID, LPCTSTR lpszAppTitle, void* pDocTemplate, BOOL bNewFrame) = 0;
	virtual ITangramDoc* OpenDocument(void* pDocTemplate, CString strFile, BOOL bNewFrame) = 0;
	virtual CTangramWndNodeProxy* OnTangramNodeInit(IWndNode* pNewNode) = 0;
	virtual CCompositorProxy* OnCompositorCreated(ICompositor* pNewCompositor) { return nullptr; }
	virtual CCompositorManagerProxy* OnCompositorManagerCreated(ICompositorManager* pNewWndPage) = 0;
	virtual void MouseMoveProxy(HWND hWnd) = 0;
	virtual void AddDoc(LONGLONG llDocID, ITangramDoc* pDoc) = 0;
	virtual void RemoveDoc(LONGLONG llDocID) = 0;
	virtual ITangramDoc* GetDoc(LONGLONG llDocID) = 0;
